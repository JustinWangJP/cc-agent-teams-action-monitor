...
| T-API-006 | get_team_stopped | チームステータス stopped 判定 | 24時間超過で stopped | 高 |

#### tasks.py テストケース

| テストID | テスト名 | テスト内容 | 期待結果 | 優先度 |
|----------|----------|-----------|----------|--------|
| T-API-007 | list_tasks_success | タスク一覧取得（正常） | 200, タスク一覧返却 | 高 |
| T-API-008 | list_tasks_empty | タスク一覧取得（データなし） | 200, 空配列返却 | 高 |
| T-API-009 | list_team_tasks | チーム別タスク取得 | 200, タスク一覧 | 高 |
| T-API-010 | get_task_success | タスク詳細取得（正常） | 200, タスク詳細 | 高 |
| T-API-011 | get_task_not_found | タスク詳細取得（存在しない） | 404 | 高 |
| T-API-012 | get_task_stopped | タスクステータス stopped 判定 | 24時間超過で stopped | 高 |

#### messages.py テストケース（新規）

| テストID | テスト名 | テスト内容 | 期待結果 | 優先度 |
|----------|----------|-----------|----------|--------|
| T-API-013 | get_timeline_success | タイムライン取得（正常） | 200, タイムラインアイテム | 高 |
| T-API-014 | get_timeline_with_filters | フィルター付き取得 | フィルタ済み結果 | 高 |
| T-API-015 | get_timeline_search | 全文検索 | 検索結果 | 中 |
| T-API-016 | get_timeline_since | 差分取得 | since以降のデータ | 中 |

#### agents.py テストケース（新規）

| テストID | テスト名 | テスト内容 | 期待結果 | 優先度 |
|----------|----------|-----------|----------|--------|
| T-API-017 | list_agents_success | エージェント一覧取得 | 200, エージェント一覧 | 高 |
| T-API-018 | list_agents_empty | エージェントなし | 200, 空配列 | 中 |

#### websocket.py テストケース

| テストID | テスト名 | テスト内容 | 期待結果 | 優先度 |
|----------|----------|-----------|----------|--------|
| T-API-019 | ws_connect | WebSocket接続 | 接続成功 | 高 |
| T-API-020 | ws_ping_pong | ping/pong通信 | pong返却 | 高 |
| T-API-021 | ws_disconnect | 切断処理 | 接続リストから削除 | 高 |
| T-API-022 | ws_broadcast | ブロードキャスト | 全クライアント受信 | 中 |
...
#### cache_service.py テストケース（新規）

| テストID | テスト名 | テスト内容 | 期待結果 | 優先度 |
|----------|----------|-----------|----------|--------|
| T-SVC-005 | cache_get_set | キャッシュ取得・設定 | 正常動作 | 高 |
| T-SVC-006 | cache_expiry | TTL期限切れ | 期限切れでNone返却 | 高 |
| T-SVC-007 | cache_invalidate | キャッシュ無効化 | 無効化で再取得 | 高 |
| T-SVC-008 | cache_cleanup | 自動クリーンアップ | 期限切れ削除 | 中 |
...
| T-HK-003 | useTeams_polling | ポーリング動作 | 定期更新 | 高 |
| T-HK-004 | useTasks_fetch | データ取得 | tasks更新 | 高 |
| T-HK-005 | useWebSocket_connect | 接続確立 | connected | 高 |
| T-HK-006 | useWebSocket_message | メッセージ受信 | lastMessage更新 | 高 |
| T-HK-007 | useWebSocket_reconnect | 再接続 | 自動再接続 | 中 |
| T-HK-008 | useInbox_fetch | インボックス取得 | メッセージ取得 | 高 |
