...
- **Radix UI** - アクセシブルUIコンポーネント（Dialog, Select, Slider）
- **react-hot-toast** - トースト通知
- **D3 / vis-timeline** - タイムライン可視化
...
判定フロー：
1. `config.json` の `members` 配列を確認 → 空の場合は `inactive`
2. セッションログの存在確認 → なしの場合は `unknown`
3. セッションログ mtime と現在時刻の差分 → 1時間以内は `active`、超過は `stopped`
...
- `projects/{project-hash}/{session}.jsonl` - セッションログ（ディレクトリは残す）
