...
try {
    ...
    if (teamOptions.length > 1) {
        // Select the first non-empty team using correct Playwright API
    ...
    recordTest('TC-011', 'メッセージバブルが表示される',
    ...
    const hasTimestamp = pageText && (
        pageText.includes(':') || // HH:MM format
        pageText.includes('T')     // ISO format
    );
    recordTest('TC-013', 'タイムスタンプが表示される',
        hasTimestamp,
